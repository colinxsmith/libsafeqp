...
int main(int argc,char** argv)
{
	...
	char line[line_len];
	std::string attribs=(char*)"soft_m soft_L soft_U soft_b soft_l n nfac m soft_A A L U alpha bench Q gamma initial delta buy sell kappa basket tradenum revise costs min_holding min_trade ls full rmin rmax round min_lot size_lot ncomp Composites value npiece hpiece pgrad nabs A_abs mabs I_A Abs_U FC FL SV minRisk maxRisk ogamma1 mask log logfile downrisk downfactor longbasket shortbasket tradebuy tradesell zetaS zetaF shortalphacost qbuy qsell issues five ten forty ShortCostScale valuel Abs_L";
	vstring vattribs;
	vdouble linedata;
	...
	size_t nf=(size_t)vecmap["nfac"][0];
...
//	vector alpha=&vecmap["alpha"].front();
	...
	long trades=(long)vecmap["tradenum"][0];
	...
	vector Comp=getvector(vecmap,(char*)"Composites");
	if(vecmap.find("Composites")!=vecmap.end())
		Comp=&vecmap["Composites"].front();
	double value=(double)vecmap["value"][0];
	double valueL=(double)vecmap["valuel"][0];
	...
	vector A_abs=getvector(vecmap,(char*)"A_abs");
	...
	vector FC=getvector(vecmap,(char*)"FC");
	vector FL=getvector(vecmap,(char*)"FL");
	vector SV=getvector(vecmap,(char*)"SV");
